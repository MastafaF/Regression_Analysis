...
glmdata.bis <- data.frame(glmdata)

#dfapply(glmdata,favstats) to have statistical values of the parameters
...
#                             breaks = c(-Inf, 1000, 2750, 3250, 3500, 3750, 4000, 5000, Inf), 
#                             labels = c("01_<1000kg", "02_1000-2749kg", "03_2749-3249kg","04_3250-3499kg","05_3500-3749kg","06_3750-3999kg","07_4000-4999kg", "08_>=5000kg"), 
#                             right = FALSE)


## Try another cut on Weight: works best 
glmdata$weight_group <- cut(glmdata$Weight, 
                            breaks = c(-Inf, 1000, 2000, 3000, 4000, Inf), 
                            labels = c("01_<1000kg", "02_1000-2000kg", "03_2000-3000kg","04_3000-4000kg", "05_>=5000kg"), 
                            right = FALSE)


glmdata.bis$weight_group <- cut(glmdata$Weight, 
                            breaks = c(-Inf, 1000, 2000, 3000, 4000, Inf), 
                            labels = c("01_<1000kg", "02_1000-2000kg", "03_2000-3000kg","04_3000-4000kg", "05_>=5000kg"), 
                            right = FALSE)
...
res <- aggregate(glmdata[,7:8],by=list(year = glmdata$RiskYear
                                       ), FUN=sum, na.rm=TRUE)


######### Simple estimate of Claim Cost for year 2017 ############
res_test <- data.frame(res)
res_test$AVG_cost <- res$ClaimCost/res$NoOfClaims

mean_cost = mean(res_test$AVG_cost)
mean_nb_claims = mean(res_test$NoOfClaims)

## Simple estimate of the Total cost for the year 2017: 
simple_estimate_total_cost_2017 = mean_cost*mean_nb_claims
simple_estimate_price_2017 = simple_estimate_total_cost_2017/0.9
...
## Try another cut on age : Works better 
glmdata$age_group <- cut(glmdata$VehicleAge, 
                         breaks = c(-Inf, 20, Inf), 
                         labels = c("01", "2"), 
                         right = FALSE)
...
##here in glmdata2, each row corresponds to a tractor
...
glmdata3 <- cbind(glmdata3, new.cols)  ##here in glmdata3 each row corresponds to a group of one variable
rm(new.cols) ##rm is remove
...
####################### Model testing with Likelihood ratio test -- RM = without ActivityCode ####################

model.frequency_reduced = glm(NoOfClaims ~ weight_group + Climate + age_group + offset(log(Duration)),
                              data = glmdata2, family = poisson)

library(lmtest)

## Tests whether Activity Code regressor is significant or not. 
likelihood.test_frequency = lrtest ( model.frequency, model.frequency_reduced )
AIC.test_frequency = AIC (model.frequency, model.frequency_reduced)
BIC.test_frequency = BIC (model.frequency, model.frequency_reduced)
...
model.severity_reduced = glm(avgclaim ~ weight_group + Climate + age_group ,
                             data = glmdata2[glmdata2$avgclaim>0,], family = Gamma("log"), weight=NoOfClaims)
...
## Tests whether Activity Code regressor is significant or not. 
likelihood.test_severity = lrtest ( model.severity, model.severity_reduced )
AIC.test_severity = AIC (model.severity, model.severity_reduced)
BIC.test_severity = BIC ( model.severity, model.severity_reduced)
...
####################### BASE LEVEL YEAR 2017 #######################

# simple estimate: 
product.factors = prod(glmdata3$rels.risk)
base.level_year_2017 = simple_estimate_price_2017/product.factors

# plot Nb of claims vs Years 

plot(res_test$year, res_test$NoOfClaims)

# Create better estimate with GLM: 

## 
model.nb_claims <-
  glm(NoOfClaims ~ year,
      data = res_test, family = poisson)

df <- data.frame(2017)
x <- c("year")
colnames(df) <- x

NOC.2017 = predict(model.nb_claims, df, se.fit = TRUE)

# value of NOC in 2017; checked in console by applying exp to year 2017 for estimate intercept and slope
NOC.2017_value = c(41.80) 

## 
model.claim_cost <- glm(ClaimCost ~ year,
      data = res_test, family = Gamma("log"), weight=NoOfClaims )



ECC.2017 = predict(model.claim_cost, df, se.fit = TRUE)

ECC.2017_value = c(1108083) 

## total cost in 2017
total.cost_2017_GLM = ECC.2017_value*NOC.2017_value
price_estimate_GLM = total.cost_2017_GLM/0.9

base.level_GLM = price_estimate_GLM/product.factors

plot(res_test$year, res_test$ClaimCost)
